...
TEST_CASE("Resource Manager", "[golxzn][core][tests]") {
	...
	SECTION("image") {
		...
		REQUIRE(core::res_man::save_image("user://original.jpg", jpg) == true);
		jpg->flip(core::types::image::flip_direction::both);
		REQUIRE(core::res_man::save_image("user://flipped_both.jpg", jpg) == true);
		jpg->flip(core::types::image::flip_direction::vertical);
		REQUIRE(core::res_man::save_image("user://flipped_vertically.tga", jpg) == true);
		jpg->flip(core::types::image::flip_direction::horizontal);
		REQUIRE(core::res_man::save_image("user://flipped_horizontally.bmp", jpg) == true);
	}
}
...
TEST_CASE("Resource Manager", "[golxzn][core][benchmarks]") {
	...
	BENCHMARK("save_image: user://images/test.jpg") { return for_bench.save_image("user://images/test.jpg"); };
	BENCHMARK("flip v: true,  h: false")  { return for_bench.flip(true, false);  };
	BENCHMARK("save_image: user://images/test.png") { return for_bench.save_image("user://images/test.png"); };
	BENCHMARK("flip v: false, h: true ")  { return for_bench.flip(false, true);  };
	BENCHMARK("save_image: user://images/test.bmp") { return for_bench.save_image("user://images/test.bmp"); };
	BENCHMARK("flip v: true,  h: true ")  { return for_bench.flip(true, true);  };
	BENCHMARK("save_image: user://images/test.tga") { return for_bench.save_image("user://images/test.tga"); };
}
